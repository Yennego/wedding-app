...
export default function BridalPartySlider({ images, heightClass = 'h-[300px] md:h-[420px]' }: { images: string[]; heightClass?: string }) {
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden shadow-xl bridal-banner">
      <Carousel opts={{ align: 'start', loop: true }} setApi={setApi} className="w-full">
        <CarouselContent>
          {images.map((src, idx) => (
            <CarouselItem key={idx} className="basis-1/2 sm:basis-1/3 md:basis-1/4">
